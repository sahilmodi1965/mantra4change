# TOKEN_BUDGET — Mantra4Change brain
...
The 6 skills are: `mantra4change-ingest`, `-query`, `-write`, `-feedback`, `-lint`, `-weekly-review`.

---

## Voice-layer

Loaded **only** by `mantra4change-write`. No other skill touches it.
...
|---|---:|
| **`mantra4change-write`** | **≤ 12,000 tokens** |
| `mantra4change-query` | ≤ 6,000 tokens |
| `mantra4change-ingest` | ≤ 8,000 tokens (excluding raw payload) |
| `mantra4change-feedback` | ≤ 4,000 tokens |
| `mantra4change-lint` | ≤ 3,000 tokens |
| `mantra4change-weekly-review` | ≤ 16,000 tokens (maintainer-driven, runs on $200) |
...
2. **No skill may read `wiki/voice/**`** unless it is `mantra4change-write`.
