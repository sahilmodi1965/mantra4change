...
- Does NOT fire on knowledge questions (→ `mantra4change-query`) or ingestion (→ `mantra4change-ingest`).
...
5. **Up to 6 `wiki/**.md` files** drilled via wikilink match against the user's brief.
...
2. **Assemble context** per the read order above. If any read pushes over 12K, drop oldest exemplar first, then drop the lowest-relevance wiki article. Voice + styleguide is never dropped.
3. **Produce the draft.** Apply voice rules. Footnote each substantive claim with the `wiki/sources/<slug>` it draws from.
