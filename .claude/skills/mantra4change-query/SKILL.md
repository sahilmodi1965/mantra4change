...
- Does NOT fire on draft requests (→ `mantra4change-write`) or correction signals (→ `mantra4change-feedback`).
...
- Do not invent claims. If the brain doesn't cover it, say so explicitly — *"That's not in the brain yet. Want me to flag it as a topic to add?"* — and offer to route to `mantra4change-feedback` if the user wants the gap captured.
- Do not read `wiki/voice/**` (that's `mantra4change-write`'s budget).
